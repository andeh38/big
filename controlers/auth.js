...
exports.register = async (req, res) => {
    try {
        ...
        if (userWithUsername) return res.status(401).json({message: 'User with this username already exist.'});
...
exports.login = async (req, res) => {
    try {
        ...
        if(!email) return res.status(401).json({message: 'Enter email to login'})

        if(!password) return res.status(401).json({message: 'Enter password to login'})
